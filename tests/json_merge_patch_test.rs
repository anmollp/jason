...
fn test_merge_patch_nested_object() {
    ...
    assert_eq!(
        target.pointer("/address/street"),
        Some(&JsonValue::String("123 Main St".to_string()))
    );
    assert_eq!(
        target.pointer("/address/city"),
        Some(&JsonValue::String("Los Angeles".to_string()))
    );
}
...
fn test_merge_patch_deeply_nested() {
    ...
    patch_level3.insert(
        "country".to_string(),
        JsonValue::String("Canada".to_string()),
    );
    ...
    assert_eq!(
        target.pointer("/address/info/country"),
        Some(&JsonValue::String("Canada".to_string()))
    );
}
...
fn test_merge_patch_replace_object_with_value() {
    ...
    patch_map.insert(
        "data".to_string(),
        JsonValue::String("simple string".to_string()),
    );
    ...
    assert_eq!(
        target.pointer("/data"),
        Some(&JsonValue::String("simple string".to_string()))
    );
}
...
fn test_merge_patch_replace_non_object_target() {
    ...
    assert_eq!(
        target.pointer("/name"),
        Some(&JsonValue::String("new".to_string()))
    );
}
